...
const target = process.env.AGENTDOX_DATABASE_URL || resolve(root, 'data', 'agentdox.db');
if (!AgentDox.isPostgres(target)) mkdirSync(resolve(root, 'data'), { recursive: true });

const dox = await AgentDox.open(target);

if ((await dox.memory.count()) > 0 || (await dox.docs.list({ limit: 1 })).length > 0) {
  console.log(`[seed] ${dox.storage} already contains data — skipping (empty it to reset).`);
  await dox.close();
  process.exit(0);
}
...
const mem = [
  ...
  { content: 'Storage is SQLite for one machine or Postgres for several instances, behind one store API.', category: scope, target: 'agentdox', importance: 0.75, tags: ['storage'] },
...
for (const d of docs) {
  await dox.docs.create(d);
  console.log(`[seed] doc: ${d.slug}`);
}

// ---- Session ----
const s = await dox.sessions.create({ scope, title: 'Kickoff' });
await dox.sessions.append(s.id, { role: 'user', content: 'How does agentdox give an agent context?' });
await dox.sessions.append(s.id, { role: 'assistant', content: 'You assemble it on demand: memory + docs + recent conversation, scoped to the agent.' });
console.log(`[seed] session: ${s.id}`);

// ---- Demonstrate context ----
const ctx = await dox.context.assemble({ scope, query: 'how is context assembled', memoryLimit: 6, docsLimit: 2, sessionLimit: 10 });
console.log('\n[seed] === Assembled context (prompt block) ===\n');
console.log(ctx.prompt);
console.log(`\n[seed] done. Total memory: ${await dox.memory.count()}, docs: ${(await dox.docs.list({ limit: 50 })).length}, sessions: ${(await dox.sessions.list()).length}`);
await dox.close();
